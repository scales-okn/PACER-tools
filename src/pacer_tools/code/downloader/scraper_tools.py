...
def get_document_links(docket_table, get_att=False, filter_fn=None, wanted_doc_nos={}):
    ...
    for row in docket_table.select("tr")[1:]:
        ...
        # Decide flow
        get_line_doc = (not bool(wanted_doc_nos)) or (
            "0" in wanted_doc_nos.get(td_line_no, [])
        )

        get_line_atts = (not bool(wanted_doc_nos) and get_att) or len(
            [x for x in wanted_doc_nos.get(td_line_no, []) if x != "0"]
        ) > 0
        ...
        if get_line_atts:
            ...
            atags = [
                a
                for a in atags
                if a.attrs.get("href").split("/")[-1] not in line_doc_ids
            ]
            ...
            if wanted_doc_nos:
                atags = [
                    a
                    for a in atags
                    if a.text.strip() in wanted_doc_nos.get(td_line_no, [])
                ]
...
def is_logged_in(browser):
    ...
    return "logout" in [
        link.text.lower().replace(" ", "").strip() for link in nav_links
    ]
...
def check_time_continue(start, end, current_ind, logging=None):
    ...
    if get_time_central().hour in no_run_hours:
        print(
            f"Scraping operating outside of running hours, terminating (at file index {current_ind})"
        )
        if logging:
            logging.info(
                f"Scraping operating outside of running hours, terminating (at file index {current_ind})"
...
def parse_docket_input(query_results, docket_input, case_type, court, logging=None):
    ...
    # CSV case
        ...
        keepcols = [
            col for col in ("case_no", "latest_date", "def_no") if col in df.columns
        ]
...
def build_case_list_from_queries(query_htmls, case_type, court):
    ...
    cases = (
        pd.concat(case_ids).map(ftools.clean_case_id).drop_duplicates()
        if case_ids
        else []
    )
    #
...
def compile_query_list(
    query_dir, court, case_type=None, output=True, drop_undownloaded=True
):
    """
    ...
    if drop_undownloaded:
        df["file_exists"] = df.apply(
            lambda x: Path(ftools.get_expected_path(x["ucid"], subdir="html")).exists(),
            axis=1,
        )
        print(df)
